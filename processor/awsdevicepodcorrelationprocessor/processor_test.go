...
type mockLookup struct {
	data map[string]map[string]*podinfo.ContainerInfo
}

func newMockLookup(data map[string]map[string]*podinfo.ContainerInfo) *mockLookup {
	return &mockLookup{data: data}
}

func (m *mockLookup) GetContainerInfo(deviceID, resourceName string) *podinfo.ContainerInfo {
...
func newMockDRALookup(data map[string]map[string]*podinfo.ContainerInfo) *mockDRALookup {
	return &mockDRALookup{data: data}
}

func (m *mockDRALookup) GetDRAContainerInfo(deviceID, driverName string) *podinfo.ContainerInfo {
...
func TestProcessMetrics_CorrelatesDeviceToPod(t *testing.T) {
	lookup := newMockLookup(map[string]map[string]*podinfo.ContainerInfo{
...
func TestProcessMetrics_DRAFallbackAfterDevicePluginMiss(t *testing.T) {
	lookup := newMockLookup(map[string]map[string]*podinfo.ContainerInfo{})
	draLookup := newMockDRALookup(map[string]map[string]*podinfo.ContainerInfo{
