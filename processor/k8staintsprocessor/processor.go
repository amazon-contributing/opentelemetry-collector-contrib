...
type k8sTaintsProcessor struct {
	cfg    *Config
	logger *zap.Logger
...
func newProcessor(cfg *Config, logger *zap.Logger) *k8sTaintsProcessor {
	return &k8sTaintsProcessor{
		cfg:    cfg,
		logger: logger,
...
func resolveLocalNodeName() (string, error) {
	...
	if name == "" {
		return "", errors.New("local_mode enabled but node name is empty (K8S_NODE_NAME unset and hostname empty)")
	}
	return name, nil
}

func (p *k8sTaintsProcessor) Start(_ context.Context, _ component.Host) error {
...
func (p *k8sTaintsProcessor) Shutdown(_ context.Context) error {
...
func (p *k8sTaintsProcessor) handleNodeAdd(obj any) {
...
func (p *k8sTaintsProcessor) handleNodeUpdate(_, newObj any) {
...
func (p *k8sTaintsProcessor) handleNodeDelete(obj any) {
	node, ok := obj.(*api_v1.Node)
	if !ok {
