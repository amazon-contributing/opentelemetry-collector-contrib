...
var typ = component.MustNewType("otlpjson")
...
func TestComponentLifecycle(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name+"-shutdown", func(t *testing.T) {
			c, err := tt.createFn(t.Context(), connectortest.NewNopSettings(typ), cfg)
			require.NoError(t, err)
			err = c.Shutdown(t.Context())
			require.NoError(t, err)
		})
		t.Run(tt.name+"-lifecycle", func(t *testing.T) {
			firstConnector, err := tt.createFn(t.Context(), connectortest.NewNopSettings(typ), cfg)
			require.NoError(t, err)
			host := componenttest.NewNopHost()
			require.NoError(t, err)
			require.NoError(t, firstConnector.Start(t.Context(), host))
			require.NoError(t, firstConnector.Shutdown(t.Context()))
			secondConnector, err := tt.createFn(t.Context(), connectortest.NewNopSettings(typ), cfg)
			require.NoError(t, err)
			require.NoError(t, secondConnector.Start(t.Context(), host))
			require.NoError(t, secondConnector.Shutdown(t.Context()))
		})
	}
}
