...
func runTraceExport(t *testing.T, testConfig *Config, traces ptrace.Traces, expectedBatchesNum int) ([]receivedRequest, error) {
	...
	params := exportertest.NewNopSettings(metadata.Type)
	exporter, err := factory.CreateTraces(b.Context(), params, cfg)
	assert.NoError(t, err)
	assert.NoError(t, exporter.Start(b.Context(), componenttest.NewNopHost()))
	defer func() {
		assert.NoError(t, exporter.Shutdown(b.Context()))
	}()

	err = exporter.ConsumeTraces(b.Context(), traces)
	assert.NoError(t, err)
	var requests []receivedRequest
	for {
		select {
...
func runLogExport(t *testing.T, cfg *Config, ld plog.Logs, expectedBatchesNum int) ([]receivedRequest, error) {
	...
	params := exportertest.NewNopSettings(metadata.Type)
	exporter, err := NewFactory().CreateLogs(b.Context(), params, cfg)
	assert.NoError(t, err)
	assert.NoError(t, exporter.Start(b.Context(), componenttest.NewNopHost()))
	defer func() {
		assert.NoError(t, exporter.Shutdown(b.Context()))
	...
	err = exporter.ConsumeLogs(b.Context(), ld)
	assert.NoError(t, err)
...
func Test_PushMetricsData_Histogram_NaN_Sum(t *testing.T) {
	...
	permanentErrors := c.pushMetricsDataInBatches(b.Context(), metrics, map[string]string{})
...
func TestErrorReceived(t *testing.T) {
	...
	err = exporter.ConsumeTraces(b.Context(), td)
	select {
	case <-rr:
	case <-time.After(5 * time.Second):
...
func TestHeartbeatStartupFailed(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.EqualError(t,
		exporter.Start(b.Context(), componenttest.NewNopHost()),
	...
	for _, test := range tests {
		for _, disabled := range []bool{true, false} {
			t.Run(test.name(disabled), func(t *testing.T) {
				...
				err := c.pushLogData(b.Context(), test.logs)
				assert.NoError(t, err)
			})
		}
	}
...
func Test_pushLogData_InvalidLog(t *testing.T) {
	...
	err := c.pushLogData(b.Context(), logs)
...
func Test_pushLogData_ShouldAddResponseTo400Error(t *testing.T) {
	...
	err := splunkClient.pushLogData(b.Context(), logs)
...
func Test_pushLogData_ShouldAddHeadersForProfilingData(t *testing.T) {
	...
	err := c.pushLogData(b.Context(), logs)
	require.NoError(t, err)
	err = c.pushLogData(b.Context(), profilingData)
	require.NoError(t, err)
