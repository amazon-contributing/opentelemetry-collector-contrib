...
var typ = component.MustNewType("filesystem")
...
func TestComponentLifecycle(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name+"-shutdown", func(t *testing.T) {
			c, err := tt.createFn(t.Context(), scrapertest.NewNopSettings(typ), cfg)
			require.NoError(t, err)
			err = c.Shutdown(t.Context())
			require.NoError(t, err)
		})
		t.Run(tt.name+"-lifecycle", func(t *testing.T) {
			firstRcvr, err := tt.createFn(t.Context(), scrapertest.NewNopSettings(typ), cfg)
			require.NoError(t, err)
			host := componenttest.NewNopHost()
			require.NoError(t, err)
			require.NoError(t, firstRcvr.Start(t.Context(), host))
			require.NoError(t, firstRcvr.Shutdown(t.Context()))
			secondRcvr, err := tt.createFn(t.Context(), scrapertest.NewNopSettings(typ), cfg)
			require.NoError(t, err)
