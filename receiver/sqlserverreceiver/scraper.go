...
import (
	...
	"github.com/open-telemetry/opentelemetry-collector-contrib/receiver/sqlserverreceiver/internal/priorityqueue"
)
...
func (s *sqlServerScraperHelper) ScrapeMetrics(ctx context.Context) (pmetric.Metrics, error) {
	...
	case getSQLServerSessionStatesQuery(s.config.InstanceName):
		err = s.recordSessionStates(ctx)
	default:
...
func (s *sqlServerScraperHelper) recordDatabaseIOMetrics(ctx context.Context) error {
	...
	const rgReadStallMsKey = "rg_read_stall_ms"
	const rgWriteStallMsKey = "rg_write_stall_ms"
	...
	for i, row := range rows {
		...
		// Record Resource Governor I/O queue wait times (SQL Server 2012+)
		if _, ok := row[rgReadStallMsKey]; ok {
			if val, err = retrieveFloat(row, rgReadStallMsKey); err == nil {
				s.mb.RecordSqlserverDatabaseIoStallQueuedDataPoint(now, val.(float64)/1e3, row[physicalFilenameKey], row[logicalFilenameKey], row[fileTypeKey], metadata.AttributeDirectionRead)
			}
		}
		if _, ok := row[rgWriteStallMsKey]; ok {
			if val, err = retrieveFloat(row, rgWriteStallMsKey); err == nil {
				s.mb.RecordSqlserverDatabaseIoStallQueuedDataPoint(now, val.(float64)/1e3, row[physicalFilenameKey], row[logicalFilenameKey], row[fileTypeKey], metadata.AttributeDirectionWrite)
			}
		}
...
func (s *sqlServerScraperHelper) recordDatabasePerfCounterMetrics(ctx context.Context) error {
	...
	const forwardedRecordsPerSec = "Forwarded Records/sec"
	...
	for i, row := range rows {
		...
		case activeTransactions:
			// "Active Transactions" is reported per database; only the server-wide
			// "Total" instance is recorded to avoid duplicate data points.
			if row[instanceKey] != "Total" {
				continue
			}
			val, err := retrieveInt(row, valueKey)
			if err != nil {
				err = fmt.Errorf("failed to parse valueKey for row %d: %w in %s", i, err, activeTransactions)
				errs = append(errs, err)
			} else {
				s.mb.RecordSqlserverTransactionActiveCountDataPoint(now, val.(int64))
			}
		case forwardedRecordsPerSec:
			val, err := retrieveFloat(row, valueKey)
			if err != nil {
				err = fmt.Errorf("failed to parse valueKey for row %d: %w in %s", i, err, forwardedRecordsPerSec)
				errs = append(errs, err)
			} else {
				s.mb.RecordSqlserverForwardedRecordsRateDataPoint(now, val.(float64))
			}
...
func (s *sqlServerScraperHelper) recordDatabaseQueryTextAndPlan(ctx context.Context) (pcommon.Resource, error) {
	...
	for i, row := range rows {
		...
		// Retrieve and obfuscate the raw query plan
		...
		// Process query plan through caching and compression (always enabled)
		processedQueryPlan := s.processQueryPlan(queryHashVal, queryPlanHashVal, queryPlanVal.(string))
...
func (s *sqlServerScraperHelper) recordDatabaseSampleQuery(ctx context.Context) (pcommon.Resource, error) {
	...
	for _, row := range rows {
		...
		// Session status from sys.dm_exec_sessions.STATUS
		...
		waitCategoryVal := categorizeWaitType(waitTypeVal)
...
// compressQueryPlan compresses an XML query plan using gzip and encodes it as base64.
// compressQueryPlan compresses an XML query plan using gzip and encodes it as base64.
// Query plans are always compressed to fit within CloudWatch Logs 1MB limit.
// Returns the compressed and encoded plan, and any error encountered.
func (s *sqlServerScraperHelper) compressQueryPlan(plan string) (string, error) {
	if plan == "" {
		return "", nil
	}
	...
	_, err := gzipWriter.Write([]byte(plan))
	if err != nil {
		s.logger.Warn("Failed to write to gzip writer for query plan compression", zap.Error(err))
		return plan, err
	}

	err = gzipWriter.Close()
	if err != nil {
		s.logger.Warn("Failed to close gzip writer for query plan compression", zap.Error(err))
		return plan, err
	}
	...
	originalSize := len(plan)
	compressedSize := len(compressed)
	compressionRatio := float64(originalSize) / float64(compressedSize)

	s.logger.Debug("Query plan compressed",
...
// processQueryPlan handles query plan caching and compression.
// It first checks the cache, then applies compression (always enabled), and ensures
// the plan doesn't exceed MaxQueryPlanSize. Returns the processed plan.
func (s *sqlServerScraperHelper) processQueryPlan(queryHash, queryPlanHash, rawPlan string) string {
	if rawPlan == "" {
		return ""
	}

	// Create cache key from query hash and plan hash
	cacheKey := queryHash + "-" + queryPlanHash + "-plan"

	// Check cache first
	if s.queryPlanCache != nil && s.config.QueryPlanCacheSize > 0 {
		if cachedPlan, ok := s.queryPlanCache.Get(cacheKey); ok {
			s.logger.Debug("Query plan retrieved from cache",
				zap.String("query_hash", queryHash),
				zap.String("query_plan_hash", queryPlanHash))
			return cachedPlan // Cached plans are already compressed
		}
	}

	// Compress the plan (always enabled)
	processedPlan, err := s.compressQueryPlan(rawPlan)
	if err != nil {
		s.logger.Warn("Query plan compression failed, using original plan",
			zap.String("query_hash", queryHash),
			zap.Error(err))
		processedPlan = rawPlan
	}

	// Apply size limit if configured
	if s.config.MaxQueryPlanSize > 0 && len(processedPlan) > s.config.MaxQueryPlanSize {
		s.logger.Warn("Query plan exceeds MaxQueryPlanSize, truncating",
			zap.String("query_hash", queryHash),
			zap.Int("original_size", len(processedPlan)),
			zap.Int("max_size", s.config.MaxQueryPlanSize))
		processedPlan = processedPlan[:s.config.MaxQueryPlanSize]
	}

	// Cache the processed plan
	if s.queryPlanCache != nil && s.config.QueryPlanCacheSize > 0 {
		s.queryPlanCache.Add(cacheKey, processedPlan)
		s.logger.Debug("Query plan added to cache",
			zap.String("query_hash", queryHash),
			zap.String("query_plan_hash", queryPlanHash),
			zap.Int("size_bytes", len(processedPlan)))
	}

	return processedPlan
}

// categorizeWaitType maps a SQL Server wait_type to a coarse wait category
// (CPU, IO, Lock, Latch, Log, Network, Memory, Remote, Idle, Other). This mirrors
// the categorization used by RDS Performance Insights so DB Load can be grouped by
// wait category. An empty wait_type means the request is running on CPU.
func categorizeWaitType(waitType string) string {
	...
	case strings.HasPrefix(waitType, "LCK_M_"):
		return "Lock"
	case strings.HasPrefix(waitType, "PAGEIOLATCH_"),
		waitType == "IO_COMPLETION",
		waitType == "ASYNC_IO_COMPLETION",
		waitType == "ASYNC_DISKPOOL_LOCK":
		return "IO"
	case strings.HasPrefix(waitType, "PAGELATCH_"),
		strings.HasPrefix(waitType, "LATCH_"),
		waitType == "AUDIT_ON_DEMAND_TARGET_LOCK":
		return "Latch"
	case waitType == "WRITELOG",
		waitType == "LOGBUFFER":
		return "Log"
	case waitType == "ASYNC_NETWORK_IO":
		return "Network"
	case waitType == "CMEMTHREAD",
		waitType == "BAD_PAGE_PROCESS":
		return "Memory"
	case waitType == "OLEDB":
		return "Remote"
	case strings.HasPrefix(waitType, "BROKER_"),
...
func (s *sqlServerScraperHelper) recordSessionStates(ctx context.Context) error {
	const sessionStateKey = "session_state"
	const sessionCountKey = "session_count"

	rows, err := s.client.QueryRows(ctx)
	if err != nil {
		if !errors.Is(err, sqlquery.ErrNullValueWarning) {
			return fmt.Errorf("sqlServerScraperHelper.recordSessionStates: %w", err)
		}
		s.logger.Warn("problems encountered getting session state rows", zap.Error(err))
	}

	var errs []error
	now := pcommon.NewTimestampFromTime(time.Now())
	for i, row := range rows {
		sessionState, ok := row[sessionStateKey]
		if !ok {
			err = fmt.Errorf("row %d: missing %s", i, sessionStateKey)
			errs = append(errs, err)
			continue
		}

		sessionCount, ok := row[sessionCountKey]
		if !ok {
			err = fmt.Errorf("row %d: missing %s", i, sessionCountKey)
			errs = append(errs, err)
			continue
		}

		count, err := strconv.ParseInt(fmt.Sprintf("%v", sessionCount), 10, 64)
		if err != nil {
			err = fmt.Errorf("row %d: failed to parse session_count: %w", i, err)
			errs = append(errs, err)
			continue
		}

		rb := s.setupResourceBuilder(s.mb.NewResourceBuilder(), row)
		s.mb.RecordSqlserverSessionCountDataPoint(now, count, fmt.Sprintf("%v", sessionState))
		s.mb.EmitForResource(metadata.WithResource(rb.Emit()))
	}

	if len(rows) == 0 {
		s.logger.Debug("SQLServerScraperHelper.recordSessionStates: No rows found by query")
	}
