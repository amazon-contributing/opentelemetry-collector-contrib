...
const sqlServerSessionStatesQuery string = `
SET DEADLOCK_PRIORITY -10;
SELECT
	'sqlserver_session_states' AS [measurement]
	,CASE 
		WHEN status = 'running' THEN 'running'
		WHEN status = 'sleeping' THEN 'sleeping'
		WHEN status = 'dormant' THEN 'dormant'
		WHEN status IN ('suspended', 'background') THEN 'waiting'
		ELSE 'waiting'
...
func getSQLServerSessionStatesQuery(instanceName string) string {
	if instanceName != "" {
		whereClause := fmt.Sprintf("\tAND @@SERVERNAME = '%s'", instanceName)
		r := strings.NewReplacer("{filter_instance_name}", whereClause)
		return r.Replace(sqlServerSessionStatesQuery)
	}
