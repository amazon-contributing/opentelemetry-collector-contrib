...
func TestMetricsBuilder(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			allMetricsCount++
			mb.RecordSqlserverForwardedRecordsRateDataPoint(ts, 1)
			...
			allMetricsCount++
			mb.RecordSqlserverTransactionActiveCountDataPoint(ts, 1)
			...
			for _, mi := range allMetricsList {
				...
				case "sqlserver.deadlock.rate":
					...
					assert.Equal(t, pmetric.NumberDataPointValueTypeDouble, dp.ValueType())
					assert.InDelta(t, float64(1), dp.DoubleValue(), 0.01)
				case "sqlserver.forwarded_records.rate":
					assert.False(t, validatedMetrics["sqlserver.forwarded_records.rate"], "Found a duplicate in the metrics slice: sqlserver.forwarded_records.rate")
					validatedMetrics["sqlserver.forwarded_records.rate"] = true
					assert.Equal(t, pmetric.MetricTypeGauge, mi.Type())
					assert.Equal(t, 1, mi.Gauge().DataPoints().Len())
					assert.Equal(t, "Number of records per second fetched through forwarded record pointers.", mi.Description())
				...
				case "sqlserver.latch.wait.rate":
					...
					assert.Equal(t, pmetric.MetricTypeGauge, mi.Type())
					assert.Equal(t, 1, mi.Gauge().DataPoints().Len())
					assert.Equal(t, "Number of latch requests per second that could not be granted immediately.", mi.Description())
				...
				case "sqlserver.transaction.active.count":
					assert.False(t, validatedMetrics["sqlserver.transaction.active.count"], "Found a duplicate in the metrics slice: sqlserver.transaction.active.count")
					validatedMetrics["sqlserver.transaction.active.count"] = true
					assert.Equal(t, pmetric.MetricTypeGauge, mi.Type())
					assert.Equal(t, 1, mi.Gauge().DataPoints().Len())
					assert.Equal(t, "The number of currently active transactions.", mi.Description())
