...
- `CHANGELOG-AWS.md` is intended for cwa team to track the changes.
### When to add a Changelog Entry

When ever we make any changes to the contrib repo, before raising the pull request we have to create a yaml file in .changelog-aws referencing the template in the directory.
...
The [CHANGELOG-AWS.md](./CHANGELOG-AWS.md) files in this repo is autogenerated from `.yaml` files in the `./.chloggen-aws` directory.
...
3. Run `make chlog-validate-aws` to ensure the new file is valid
4. Commit and push the file

Alternately, copy `./.chloggen-aws/TEMPLATE.yaml`, or just create your file from scratch.
